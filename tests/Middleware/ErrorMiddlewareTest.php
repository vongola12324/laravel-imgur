...
{
    public function testNothingHappenOnOkResponse()
    {
    ...
    public function testRateLimitClient()
    {
        ...
        $this->expectExceptionMessage(
            'No application credits available. The limit is 10 and will be reset at 2015-09-04'
        );
        $mock = new MockHandler(
            [
                new Response(
    ...
    public function testErrorOverCapacity()
    {
        ...
        $this->expectExceptionMessage(
            'Request failed with: "Imgur is temporarily over capacity. Please try again later."'
        );
        $mock = new MockHandler(
            [
                new Response(
    ...
    public function testErrorUploadingTooFast()
    {
        $this->expectException(ErrorException::class);
        $this->expectExceptionMessage(
            'Request to: /3/image.json failed with: "You are uploading too fast. Please wait 59 more minutes."'
        );
        $mock = new MockHandler(
            [
                new Response(
                    ...
                    json_encode(
                        [
                            ...
                            'data'    => [
                                'error'   => [
                                    'code'      => 429,
                                    'message'   => 'You are uploading too fast. Please wait 59 more minutes.',
                                    'type'      => 'ImgurException',
                                    'exception' => [],
                                ],
                ...
                new Response(
                    ...
                    json_encode(
                        [
                            ...
                            'success' => false,
                            'data'    => [
                                'error'   => ['code' => 666, 'type' => 'ImgurException', 'exception' => []],
                                'request' => '/3/image.json',
                                'method'  => 'POST',
                            ],
                        ]
